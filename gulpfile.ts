...
gulp.task('dev:server', ['build:index', 'build:vendor', 'build:dev'], $.bg('node', 'webpack/server.js'));
...
function buildIndexHtmlFile(yigiApp: IYigiConfigItem) {
    return () => {
        const fs = require('fs');
        const dust = require('dustjs-linkedin');
        const manifest = require(`./webpack/manifest/${ENV}`).default;
        const buildPublicAppDir: string = `${BUILD_PUBLIC_DIR}/browser/${yigiApp.name}`;

        const writeIndexHtmlUsingManifests = (manifest: Array<string>): Promise<any> =>

            Promise.all([
                promised(cb => fs.readFile(`${SRC_DIR}/browser/${yigiApp.name}/index.dust`, 'utf8', cb)),
                promised(cb => mkdirp(buildPublicAppDir, cb)),
            ])
                .then(([content]) => dust.compile(content, 'index'))
                .then(dust.loadSource)
                .then(() => promised(cb => dust.render('index', {manifest}, cb)))
                .then(html => promised(cb => fs.writeFile(`${buildPublicAppDir}/index.html`, html, cb)));
